...
private func _createSampleEventManager() -> HSSquareEventManager{
    ...
    let gameStartAction = HSEraEventMoneyAppendAction(title: "ゲーム開始！", appendMoneyCount: 1000000)
    em.registerEvent(HSEraEvent(title: "平成元年，その最初の日．", heiseiYear: 1, eventDescription: "小渕恵三が発表した「平成」の2文字は生乾きであったという。\n", imageName: "01_winter", action: gameStartAction))
    let buyHouse = HSEraEventAction(eventType: .bad, title: "消費税が上がる前に家を買った。", description: "￥600000失う。")
    em.registerEvent(HSEraEvent(title: "消費税がスタート．", heiseiYear: 1, eventDescription: "最初の消費税は3%でした。\n", imageName: "01_spring", action: buyHouse))
    ...
    let tokyoWolkerAction = HSEraEventMoneyAppendAction(title: "東京ウォーカーで紹介された自分の商品が売れまくった。", appendMoneyCount: 100000)
    em.registerEvent(HSEraEvent(title: "東京ウォーカー第1号の発行．", heiseiYear: 2, eventDescription: "記念すべき最初のテーマは「東京ディズニーランドと花やしき」でした。\nディズニーランドが実は1957年からあったことをみなさん知っていましたか？\n", imageName: "02_winter", action: tokyoWolkerAction))
    ...
    let cantStop = HSEraEventSkipSquareAction(title: "SMAPのCan't Stopを聞いて止まれなくなった。", skippableSquareCount: 5)
    em.registerEvent(HSEraEvent(title: "SMAPが「Can't Stop!! -LOVING-」でCDデビュー．", heiseiYear: 3, eventDescription: "国民的アイドルSMAPの時代はここから始まりました。\nあの解散騒動は衝撃的でしたね。\n", imageName: "03_summer", action: cantStop))
    ...
    let ozakiAction = HSEraEventMoneyReduceAction(title: "尾崎豊死去の影響で散財しまくった。￥5000失う。", reduceMoneyCount: 50000)
    em.registerEvent(HSEraEvent(title: "尾崎豊，死去．", heiseiYear: 4, eventDescription: "今尚カリスマ的人気を誇る歌手 尾崎豊はこの年になくなりました。\n26歳という早すぎる死に加え死因は不明とされており，大きな話題となりました。\n", imageName: "04_spring", action: ozakiAction))
    let universe = HSEraEventSkipSquareAction(title: "自分も宇宙へ行った気分になりたくさん歩いた。", skippableSquareCount: 3)
    em.registerEvent(HSEraEvent(title: "毛利衛，宇宙へ．", heiseiYear: 4, eventDescription: "史上二人目の日本人宇宙飛行士として毛利衛が宇宙に飛び立ちました。\n帰国後の会見で発した「宇宙からは国境線は見えなかった」というコメントはとても有名です。\n", imageName: "04_summer", action: universe))
    let checkersAction = HSEraEventMoneyReduceAction(title: "尾崎豊死去の影響で散財しまくった。", reduceMoneyCount: 50000)
    em.registerEvent(HSEraEvent(title: "チェッカーズ解散したことにより落ち込んで散財しまくった。", heiseiYear: 4, eventDescription: "昭和後期を席巻したバンドであるチェッカーズが解散します。\n代表曲である「ギザギザハートの子守唄」などは現在でもテレビで目にします。\n", imageName: "04_autumn", action: checkersAction))
    ...
    let playStation = HSEraEventMoneyReduceAction(title: "PlayStationを買った。", reduceMoneyCount: 40000)
    em.registerEvent(HSEraEvent(title: "初代PlayStation発売", heiseiYear: 6, eventDescription: "初代PlayStation発売。\n任天堂のハードと人気を二分するPlayStatoinシリーズの登場です。\n2019年4月22日現在で4まで出ており，最近5の開発が話題となりました。\nなお，初代PlayStationの一ヶ月前にはセガサターンが登場しています。\n", imageName: "06_autumn", action: playStation))
